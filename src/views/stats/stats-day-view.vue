...
import { Baby, ChevronLeft, ChevronRight, CircleDot, Clock3, Droplets, Heart, Moon, Plus } from 'lucide-vue-next';
import {
  fetchDailyRecords,
  fetchDailyStats,
  formatDateValue,
...
import BackfillTypeSheet from '@/components/backfill/BackfillTypeSheet.vue';
 ...
 * 日视图：保留原统计页的「单日复盘」体验。
 *
 * 展示单日汇总、明细列表，同时承载记录编辑、撤销以及按家庭角色限制操作的交互。
 */
...
// 补录相关：类型选择弹层 + BackfillDialog 展示的类型
const showBackfillSheet = ref(false);
...
/** 打开补录类型选择弹层，只读成员不允许触发 */
const openBackfillSheet = () => {
  if (babyStore.isReadOnly) {
    showToast({ message: '当前为只读成员，不能记录', type: 'fail' });
    return;
  }
  showBackfillSheet.value = true;
};

/** 选择类型后打开对应的 BackfillDialog */
const onBackfillTypeSelect = (type: BackfillType) => {
  backfillType.value = type;
};

/** 补录成功后刷新当日统计与明细 */
const onBackfillSubmitted = async () => {
...
<template>
  <div class="flex flex-1 min-h-0 flex-col overflow-hidden px-5 pb-6">
    <div class="mb-4 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button class="btn-today press flex items-center gap-1" aria-label="补录记录" @click="openBackfillSheet">
          <Plus :size="12" />
          补录
        </button>
          ...
          <van-swipe-cell v-for="record in records" :key="record.clientId || record.timestamp">
            <div
              ...
              <div class="flex w-full min-w-0 flex-col">
                ...
                <div
                  v-if="(familyStore.hasMembers && record.recorder?.nickname) || record.source === 'backfill'"
                  class="mt-1 ml-5 flex flex-wrap items-center gap-1">
                  <span
                    v-if="familyStore.hasMembers && record.recorder?.nickname"
                  ...
                  <span
                    v-if="record.source === 'backfill'"
                    class="inline-flex items-center gap-0.5 rounded-lg bg-[var(--surface-muted)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text-secondary)]"
                    aria-label="历史补录记录">
                    <Clock3 :size="10" />
                    补录
                  </span>
    ...
    <!-- 补录类型选择 + 补录弹窗 -->
    <BackfillTypeSheet v-model:visible="showBackfillSheet" @select="onBackfillTypeSelect" />
    <BackfillDialog
      v-if="backfillType"
